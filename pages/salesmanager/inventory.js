 document.addEventListener('DOMContentLoaded', function () {
      ...
      // Add event listener for Lost Items tab
      const lostItemsTab = document.querySelector('[href="#lost-items"]');
      if (lostItemsTab) {
        lostItemsTab.addEventListener('click', function() {
          fetchAndDisplayLostItems();
        });
      }
      ...
      // --- LOST ITEMS FUNCTIONALITY ---
      let lostItemsData = [];
      let lostItemsCurrentPage = 1;
      const lostItemsPageSize = 10;

      function renderLostItemsTable(items) {
        const tableBody = document.getElementById('lostItemsTableBody');
        if (!tableBody) return;

        if (!items || items.length === 0) {
          tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-secondary">No lost items found</td></tr>';
          return;
        }
        ...
        tableBody.innerHTML = items.map(item => {
          ...
          return `
            <tr>
              ...
              <td>
                <button class="btn btn-link text-secondary mb-0" onclick="viewLostItemDetails('${item._id}')" title="View Details">
      ...
      function updateLostItemsPaginationInfo(start, end, total) {
        ...
        if (showingText) {
          showingText.textContent = `Showing ${start} to ${end} of ${total} items`;
        }
        
        if (pageInfo) {
          const totalPages = Math.ceil(total / lostItemsPageSize);
          pageInfo.textContent = `Page ${lostItemsCurrentPage} of ${totalPages}`;
        }
      }

      function updateLostItemsPaginationControls() {
        const prevBtn = document.getElementById('lostItemsPrevPage');
        const nextBtn = document.getElementById('lostItemsNextPage');
        const totalPages = Math.ceil(lostItemsData.length / lostItemsPageSize);
        
        if (prevBtn) prevBtn.disabled = lostItemsCurrentPage <= 1;
        if (nextBtn) nextBtn.disabled = lostItemsCurrentPage >= totalPages;
      }

      function showLostItemsPage(page) {
        const startIndex = (page - 1) * lostItemsPageSize;
        const endIndex = startIndex + lostItemsPageSize;
        const pageItems = lostItemsData.slice(startIndex, endIndex);
        
        renderLostItemsTable(pageItems);
        updateLostItemsPaginationInfo(startIndex + 1, Math.min(endIndex, lostItemsData.length), lostItemsData.length);
        updateLostItemsPaginationControls();
      }
      ...
      async function fetchAndDisplayLostItems() {
        try {
          ...
          const tableBody = document.getElementById('lostItemsTableBody');
          if (tableBody) {
            tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Failed to load lost items</td></tr>';
          }
        }
      }

      // Function to view lost item details (placeholder for future modal)
      function viewLostItemDetails(itemId) {
        console.log('Viewing details for lost item:', itemId);
        // TODO: Implement modal to show detailed information
        alert('Lost item details feature coming soon!');
      }

      // Add event listeners for lost items pagination
      document.addEventListener('DOMContentLoaded', function() {
        const lostItemsPrevBtn = document.getElementById('lostItemsPrevPage');
        const lostItemsNextBtn = document.getElementById('lostItemsNextPage');
        
        if (lostItemsPrevBtn) {
          lostItemsPrevBtn.addEventListener('click', function() {
            if (lostItemsCurrentPage > 1) {
              lostItemsCurrentPage--;
              showLostItemsPage(lostItemsCurrentPage);
            }
          });
        }
        
        if (lostItemsNextBtn) {
          lostItemsNextBtn.addEventListener('click', function() {
            const totalPages = Math.ceil(lostItemsData.length / lostItemsPageSize);
            if (lostItemsCurrentPage < totalPages) {
              lostItemsCurrentPage++;
              showLostItemsPage(lostItemsCurrentPage);
            }
          });
